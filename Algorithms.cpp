    ...
    bool Algorithms::isConnected(const Graph &g)
{
    ...
    for (size_t i = 0; i < matrix.size(); ++i) {
        if (!visited[i]) {
            if ((isSymmetric(matrix)) && unDirectedCycleDFS(matrix, i, -1, visited, cycle)) {
                break;
            } else if (directedCycleDFS(matrix, i, visited, onStack, cycle)) {
                break;
            }
        }
    }
        ...
        std::function<bool(int)> bfs = [&](int s)
        {
            ...
            while (!q.empty())
            {
                ...
                for (int v = 0; v < numVertices; ++v)
                {
                    ...
                    {
                        color[v] = 1 - color[u];
                        sets[color[v]].insert(v);
                        q.push(v);
                    }
                    else if (matrix[u][v] != 0 && color[v] == color[u])
                    {
                        return false;
                    }
                }
    ...
    std::string Algorithms::bipartiteGraphToString(const std::vector<std::set<int>> &sets)
    {
        ...
        if (sets.empty())
        {
            oss << "Graph is not bipartite" << endl;
            return oss.str();
        }
   ...
   bool Algorithms::directedCycleDFS(const std::vector<std::vector<int>>& graph, int v, std::vector<bool>& visited, std::vector<bool>& onStack, std::vector<int>& cycle) {
    ...
    for (size_t i = 0; i < graph[v].size(); ++i) {
        if (graph[v][i] > 0) { // Check for an edge
            if (!visited[i]) {
                if (directedCycleDFS(graph, i, visited, onStack, cycle)) {
                    if (cycle.empty() || cycle.front() != v) {
                        cycle.push_back(v); // Building the cycle path
                        if (i == cycle.front()) return true; // End cycle if back to start
                    }
                    return true;
                }
            } else if (onStack[i]) {
                cycle.push_back(i); // Start the cycle path
    ...
    onStack[v] = false; // Backtrack: unmark the current vertex
    return false;
}
    ...
    for (size_t i = 0; i < graph[v].size(); ++i) {
        if (graph[v][i] > 0 && i != parent) { // Check for an edge, ensuring it's not the parent
            if (!visited[i]) {
                if (unDirectedCycleDFS(graph, i, v, visited, cycle)) {
                    return true;
                }
            } else {
                cycle.push_back(i); // Close the cycle
                return true;
            }
        }
    }
